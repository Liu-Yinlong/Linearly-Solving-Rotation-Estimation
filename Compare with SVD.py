...
if __name__=='__main__':
    ...
    err_my=np.zeros((repeat,11))
    tim_my=np.zeros((repeat,11))

    err_svd=np.zeros((repeat,11))
    tim_svd=np.zeros((repeat,11))

    noise_level=0.01*np.linspace(0,10,11)
    ...
    bp1=plt.boxplot(err_svd,positions=np.linspace(0,10,11)+0.25,widths=0.35,showfliers=False,boxprops=boxprops,patch_artist=True,whiskerprops=dict(linewidth=0.5),capprops=dict(linewidth=0.5))

    bp2=plt.boxplot(err_my,positions=np.linspace(0,10,11)-0.25,widths=0.35,showfliers=False,boxprops=boxprops,patch_artist=True,whiskerprops=dict(linewidth=0.5),capprops=dict(linewidth=0.5))
    ...
    plt.legend()
    plt.xticks(np.linspace(0,10,11), labels=[f'{0.01*x:.2f}' for x in np.linspace(0,10,11)], rotation=45)
    plt.xlabel('Noise level')
    plt.ylabel('Rotation error (deg)')
    plt.tight_layout()
    plt.grid()

    plt.savefig('outlier_free_err.svg')
