...
if __name__=='__main__':
    ...
    plt.tight_layout()
    plt.grid()
    plt.savefig('rotation_error_GPU.pdf')

    plt.figure()
    boxprops = dict(linewidth=0.2,)
    bp=plt.boxplot(tim_my,positions=np.linspace(1,9,9),widths=0.35,showfliers=False,boxprops=boxprops,patch_artist=True,whiskerprops=dict(linewidth=0.5),capprops=dict(linewidth=0.5))
    ...
    plt.tight_layout()
    plt.grid()
    plt.savefig('rotation_time_GPU.pdf')
