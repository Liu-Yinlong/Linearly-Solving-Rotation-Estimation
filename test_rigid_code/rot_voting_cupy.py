...
def get_R_voting(x,y):
    ...
    divided_num=18*5    #< divided inputs number depend on your GPU memory
...
def translation_voting(R_in,x_data_rigid,y_data_rigid):
    ...
    bins=cp.linspace(-5,5,401)  #<-------这个参数是写死的
...
def pose_est_liu(x_data_rigid,y_data_rigid):#<---if epsilon_inlier_dist<0, then no refinement
    ...
    R_est=rotation_voting_liu(x_data,y_data)
    t_est=translation_voting(R_est,x_data_rigid,y_data_rigid)

    return R_est,t_est

def test_rigid_pose(num,outlier_rate,noise_level):
    
    R_gt=Rotation.random()
    # print(R_gt.as_rotvec())
    t_gt=(np.random.random(3)*2-1)*1
    # print(t_gt)
    
    x_data=(np.random.rand(num,3)*2-1)*1

    y_data=R_gt.apply(x_data)+t_gt+noise_level*np.random.standard_normal(size=x_data.shape)

    outlier_num=round(num*(outlier_rate))
    if(outlier_num>0):
        y_data[0:outlier_num,:]=np.random.rand(outlier_num,3)*10

    T1=time.perf_counter_ns()

    #=====开始计算===============    
    R_opt,t_opt=pose_est_liu(x_data,y_data)
    #====结束计算=================
    ...
    R_err_=R_gt.inv()*Rotation.from_matrix(R_opt.get())
    R_err=R_err_.as_rotvec(degrees=True)
    
    r_err=np.linalg.norm(R_err)
    t_err=np.linalg.norm(t_opt.get()-t_gt)
    
    return tim,r_err,t_err
...
def test_rot_voting(num,outlier_rate,noise_level):
    ...
    outlier_num=int(num*(outlier_rate))
    if(outlier_num>0):
...
if __name__=='__main__':
    ...
    err_r_my=np.zeros((repeat,9))
    err_t_my=np.zeros((repeat,9))
    ...
    for ii in range(repeat):
        for jj in range(9):
            ...
            tim_my[ii,jj],err_r_my[ii,jj],err_t_my[ii,jj]=test_rigid_pose(num,outlier_level[jj],noise_level)

            print(ii,jj)

    plt.figure()
    boxprops = dict(linewidth=0.2,)
    bp=plt.boxplot(err_r_my,positions=np.linspace(1,9,9),widths=0.35,showfliers=True,boxprops=boxprops,patch_artist=True,whiskerprops=dict(linewidth=0.5),capprops=dict(linewidth=0.5))
    ...
    plt.tight_layout()
    plt.grid()
    plt.savefig('rigid_error_r_GPU.pdf')

    plt.figure()
    boxprops = dict(linewidth=0.2,)
    bp=plt.boxplot(err_t_my,positions=np.linspace(1,9,9),widths=0.35,showfliers=True,boxprops=boxprops,patch_artist=True,whiskerprops=dict(linewidth=0.5),capprops=dict(linewidth=0.5))
    ...
    plt.xticks(np.linspace(1,9,9), labels=[f'{x:.2f}' for x in outlier_level], rotation=45)
    plt.xlabel('outlier level')
    plt.ylabel('Transalation error ')
    plt.tight_layout()
    plt.grid()
    plt.savefig('rigid_error_t_GPU.pdf')

    plt.figure()
    boxprops = dict(linewidth=0.2,)
    bp=plt.boxplot(tim_my,positions=np.linspace(1,9,9),widths=0.35,showfliers=True,boxprops=boxprops,patch_artist=True,whiskerprops=dict(linewidth=0.5),capprops=dict(linewidth=0.5))
    ...
    plt.tight_layout()
    plt.grid()
    plt.savefig('rigid_error_tim_GPU.pdf')
