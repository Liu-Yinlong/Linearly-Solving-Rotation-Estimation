...
def get_R_voting(x,y,k_num):
    ...
    divided_num=10       #< divided inputs number depend on your GPU memory
...
def gen_data(num=1000,num_model=3):
    ...
    return x,y,R_gt_list,t_gt_list



def pose_est_voting(x_data_rigid,y_data_rigid,k_num):#<---if epsilon_inlier_dist<0, then no refinement
    x_data_rigid=cp.asarray(x_data_rigid)
    y_data_rigid=cp.asarray(y_data_rigid)
    
    num=x_data_rigid.shape[0]
    x_data_=cp.zeros(shape=(num*(num-1)//2,3))
    y_data_=cp.zeros(shape=(num*(num-1)//2,3))
    ...
    for ii in range(num-1):
        ...
        x_data_[ind_j:ind_j+ind_k,:]=x_data_rigid[ii,:]-x_data_rigid[ii+1:,:]
        y_data_[ind_j:ind_j+ind_k,:]=y_data_rigid[ii,:]-y_data_rigid[ii+1:,:]
        
        ind_j+=ind_k
    
    t1=cp.linalg.norm(x_data_,axis=1)
    t2=cp.linalg.norm(y_data_,axis=1)

    abs_threshold=0.01

    ind=cp.abs(t1-t2)<=abs_threshold
    ind[t2<=1e-10]=False
    
    x_data_0=x_data_[ind,:]
    y_data_0=y_data_[ind,:]    
    
    x_data=x_data_0/cp.reshape(t1[ind],(-1,1))
    y_data=y_data_0/cp.reshape(t2[ind],(-1,1))
        
    R_est_list=get_R_voting(x_data,y_data,k_num)
    t_est_list=[]
    for ii,R in enumerate(R_est_list):
        t_est=translation_voting(R,x_data_rigid,y_data_rigid)
        t_est_list.append(t_est.reshape((3,1)))
        R_est_list[ii]=R.as_matrix()

    return R_est_list,t_est_list
def translation_voting(R_in_,x_data_rigid,y_data_rigid):
    R_in=cp.asarray(R_in_.as_matrix())
    x_data_rigid_trans=R_in@x_data_rigid.T
    t_opt_=y_data_rigid-x_data_rigid_trans.T
    
    t_opt=cp.zeros(3)
    
    bins=cp.linspace(-1.5,1.5,401)  #<-------这个参数是写死的
    
    H,edges=cp.histogramdd(t_opt_,(bins,bins,bins))
    
    ind_=cp.argmax(H)
    ind_xyz=cp.unravel_index(ind_,dims=H.shape)
    
    t_opt[0]=0.5*(bins[ind_xyz[0]]+bins[ind_xyz[0]+1])
    t_opt[1]=0.5*(bins[ind_xyz[1]]+bins[ind_xyz[1]+1])
    t_opt[2]=0.5*(bins[ind_xyz[2]]+bins[ind_xyz[2]+1])

    
    return t_opt.get()

def solver_svd(x,y):
    center_x=cp.mean(x,axis=0,keepdims=True)
    center_y=cp.mean(y,axis=0,keepdims=True)
    x_=x-center_x
    y_=y-center_y
    ...
    t_opt=center_y.T-R_opt@center_x.T
    return R_opt,t_opt

def pose_est_voting_refine(x_data_rigid,y_data_rigid,k_num):
    R_opt_list,t_opt_list=pose_est_voting(x_data_rigid,y_data_rigid,k_num)
    for ii in range(k_num):
        R_=cp.asarray(R_opt_list[ii])
        t_=cp.asarray(t_opt_list[ii])
        for jj in range(5):
            err_=t_+R_@x_data_rigid.T-y_data_rigid.T
            ind=cp.sqrt(cp.sum(err_*err_,axis=0))<0.01
            R_,t_=solver_svd(x_data_rigid[ind,:],y_data_rigid[ind,:])

        R_opt_list[ii]=R_.get()
        t_opt_list[ii]=t_.get()
    return R_opt_list,t_opt_list  

def test_pose_voting(num):
    ...
    for ii in range(len(num_model)):
        for jj in range(repeat_time):
                ...
                R_opt_list,t_opt_list=pose_est_voting(x,y,k_num=num_model[ii])
    ...
    np.savetxt(f'./results/r_err-{num}.txt',ave_err_r)
    np.savetxt(f'./results/t_err-{num}.txt',ave_err_t)
...
def test_pose_voting_w_refine(num):
    ...
    for ii in range(len(num_model)):
        for jj in range(repeat_time):
                ...
                T1=time.perf_counter_ns()
                R_opt_list,t_opt_list=pose_est_voting_refine(x,y,k_num=num_model[ii])
                T2=time.perf_counter_ns()
    ...
    np.savetxt(f'./results/tim-{num}_refine.txt',tim)
    np.savetxt(f'./results/r_err-{num}_refine.txt',ave_err_r)
    np.savetxt(f'./results/t_err-{num}_refine.txt',ave_err_t)
    

    tim=np.loadtxt(f'./results/tim-{num}_refine.txt')
    ave_err_r=np.loadtxt(f'./results/r_err-{num}_refine.txt')
    ave_err_t=np.loadtxt(f'./results/t_err-{num}_refine.txt')
    ...
    plt.tight_layout()
    plt.savefig(f'multi-model-{num}_refine.pdf')
...
if __name__=='__main__':
    ...
    # test_pose_voting_w_refine(num=200)
    # test_pose_voting(num=200)
    ...
    positions_1=np.arange(len(num_model))+2-0.25
    positions_2=np.arange(len(num_model))+2+0.25
    ...
    plt.bar(positions_1,np.median(tim,axis=0),width=0.4,color='green')
    plt.bar(positions_2,np.median(tim_refine,axis=0),width=0.4,color='royalblue')
    ...
    bp=plt.boxplot(ave_err_r,positions=positions_1,
                   showfliers=True,patch_artist=True,widths=0.38, flierprops=flierprops)
    for f1 in bp['boxes']:
        f1.set_facecolor('green')
    f1.set_label('No refinement')
    
    bp2=plt.boxplot(ave_err_r_refine,positions=positions_2,
                    showfliers=True,patch_artist=True,widths=0.38, flierprops=flierprops)
    for f2 in bp2['boxes']:
        f2.set_facecolor('royalblue')
    f2.set_label('With refinement')
    ...
    bp=plt.boxplot(ave_err_t,positions=positions_1,showfliers=True,patch_artist=True,widths=0.38, flierprops=flierprops)
    for f1 in bp['boxes']:
        f1.set_facecolor('green')
    # f1.set_label('No refinement')
    
    bp=plt.boxplot(ave_err_t_refine,positions=positions_2,showfliers=True,patch_artist=True,widths=0.38, flierprops=flierprops)
    for f2 in bp['boxes']:
        f2.set_facecolor('royalblue')
    # f2.set_label('With refinement')
    ...
    plt.grid()
    # plt.yscale('log')
    plt.gca().ticklabel_format(axis='y',style='sci',scilimits=[0.1,1])
    plt.xticks(ticks=positions,labels=positions)
    
    
    fig.legend(handles={f1,f2},labels={'No refinement','With refinement'},loc='outside upper center',ncols=3,frameon=True,bbox_to_anchor=(0., 1.1, 1., .102))


    plt.tight_layout(pad=0.1,h_pad=0.015,w_pad=0.5)
    plt.savefig('multi-rigid.pdf')
