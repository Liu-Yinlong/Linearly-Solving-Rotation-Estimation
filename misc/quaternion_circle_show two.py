...
def vtk_show(x_1,y_1,x_2,y_2):
    ...
    pt.open_gif('quaternion_circle_two.gif')
...
if __name__=='__main__':
    ...
    x_1=x_/scipy.linalg.norm(x_,axis=1).reshape((-1,1))
    y_1=gt.apply(x_1)
    
    gt=Rotation.from_quat([0, 0.6, -0.8, 0.4])
    num=20
    x_=np.random.random((num,3))*2-1
    x_2=x_/scipy.linalg.norm(x_,axis=1).reshape((-1,1))
    y_2=gt.apply(x_2)

    
    vtk_show(x_1,y_1,x_2,y_2)           #<----using pyvista    to show quaternion circles
